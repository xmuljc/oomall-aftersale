...
import cn.edu.xmu.oomall.payment.domain.DivPayTransDao;
...
import cn.edu.xmu.oomall.payment.infrastructure.mapper.generator.po.DivPayTransPo;
...
public class DivPayTrans extends Transaction{
    ...
    public void adjust(UserToken user) {
        if (!this.allowStatus(DivPayTrans.SUCCESS) || !this.status.equals(WRONG)){
            ...
            this.divPayTransDao.update(trans, user);
        }
    }
     ...
     * @return
     */
    public void refund(RefundTrans refundTrans, UserToken user){
        //需要先调用分账回退API
        ...
        try{
            ...
            this.divRefundTransDao.update(updateTrans, user);
        }catch (Exception e){
